...
use rustconn_core::import::{
    ...
    ImportSource, ImportWarning, LibvirtDaemonImporter, LibvirtXmlImporter, MRemoteNgImporter,
    MobaXtermImporter, PuttyImporter, RdmImporter, RdpFileImporter, RemminaImporter,
    RoyalTsImporter, SecureCrtImporter, SshConfigImporter, VirtViewerImporter,
};
...
impl ImportDialog {
    pub(super) fn create_source_page() -> (GtkBox, ListBox) {
        ...
        let sources: Vec<(&str, String, String, bool)> = vec![
            ...
            (
                "mremoteng_file",
                i18n("mRemoteNG (confCons.xml)"),
                i18n("Import from an mRemoteNG connections file"),
                true,
            ),
            (
                "rdp_file",
    ...
    /// Handles importing from an mRemoteNG `confCons.xml` connections file
    #[expect(
        clippy::too_many_arguments,
        reason = "function parameters mirror upstream API or struct fields 1:1; bundling into a struct only restates the field list"
    )]
    pub(super) fn handle_mremoteng_file_import(
        parent_window: Option<&gtk4::Window>,
        stack: &Stack,
        progress_bar: &ProgressBar,
        progress_label: &Label,
        ...
        let file_dialog = gtk4::FileDialog::builder()
            .title(i18n("Select mRemoteNG Connections File"))
            .modal(true)
            .build();

        let filter = gtk4::FileFilter::new();
        filter.add_pattern("*.xml");
        filter.set_name(Some(&i18n("mRemoteNG Connections (*.xml)")));
        let filters = gtk4::gio::ListStore::new::<gtk4::FileFilter>();
        filters.append(&filter);
        file_dialog.set_filters(Some(&filters));
            ...
            move |file_result| {
                if let Ok(file) = file_result {
                    if let Some(path) = file.path() {
                        ...
                        let importer = MRemoteNgImporter::with_paths(vec![path.clone()]);
                        let result = Self::import_or_error(
                            importer.import_from_path(&path),
                            "mRemoteNG",
                        );
